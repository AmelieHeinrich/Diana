...
void main()
{
    vec3 albedo     = pow(texture(albedoMap, fTexCoords).rgb, vec3(2.2));
    float metallic  = texture(metallicMap, fTexCoords).r;
    float roughness = texture(roughnessMap, fTexCoords).r;
    float ao        = texture(aoMap, fTexCoords).r;
    ...
    float distance = length(lightPosition - fWorldPos);
